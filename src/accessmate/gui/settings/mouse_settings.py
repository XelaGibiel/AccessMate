...
class MouseSettingsWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self._enabled_cb.setChecked(self._settings.get("enabled", False))
        ...
        self._centering_delay.valueChanged.connect(lambda v: self._save("centering_delay", float(v)))
        ...
        self._centering_countdown.setValue(int(self._settings.get("centering_countdown", 3)))
        self._centering_countdown.valueChanged.connect(lambda v: self._save("centering_countdown", v))
        centering_form.addRow(tr("module.mouse.centering.countdown"), self._centering_countdown)

        self._centering_tolerance = QSpinBox()
        self._centering_tolerance.setRange(0, 500)
        self._centering_tolerance.setSuffix(" px")
        self._centering_tolerance.setValue(int(self._settings.get("centering_tolerance", 50)))
        self._centering_tolerance.valueChanged.connect(lambda v: self._save("centering_tolerance", v))
        centering_form.addRow(tr("module.mouse.centering.tolerance"), self._centering_tolerance)

        self._centering_hotkey = HotkeyEdit(self._settings.get("centering_hotkey", ""))
        self._centering_hotkey.key_changed.connect(lambda k: self._save("centering_hotkey", k))
        centering_form.addRow("Manual centering key", self._centering_hotkey)
        ...
        self._precision_cb.setChecked(self._settings.get("precision_mode_enabled", False))
        self._precision_cb.toggled.connect(lambda v: self._save("precision_mode_enabled", v))
        ...
        self._precision_slider.valueChanged.connect(lambda v: self._save("precision_speed", v))
        ...
        self._precision_hotkey.key_changed.connect(lambda k: self._save("precision_hotkey", k))
        precision_form.addRow("Precision mode key", self._precision_hotkey)
        ...
        self._clicklock_cb.toggled.connect(lambda v: self._save("click_lock_enabled", v))
        clicklock_form.addRow(tr("module.mouse.click_lock.enabled"), self._clicklock_cb)

        self._clicklock_hotkey = HotkeyEdit(self._settings.get("clicklock_hotkey", ""))
        self._clicklock_hotkey.key_changed.connect(lambda k: self._save("clicklock_hotkey", k))
        clicklock_form.addRow("Click-Lock key", self._clicklock_hotkey)
        ...
        self._kb_left.key_changed.connect(lambda k: self._save("keyboard_click_left", k))
        ...
        self._kb_right.key_changed.connect(lambda k: self._save("keyboard_click_right", k))
        ...
        self._kb_double.key_changed.connect(lambda k: self._save("keyboard_click_double", k))
        ...
        self._zones_cb.toggled.connect(lambda v: self._save("screen_zones_enabled", v))
        ...
        self._update_enabled_state(self._settings.get("enabled", False))
